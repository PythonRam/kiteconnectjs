...
var KiteConnect = function(params) {
	...
	var defaults = {
		...
		"debug": false,
		"timeout": 7000,
		"historical": "https://kitecharts-aws.zerodha.com"
	};
	self.historical = params.historical || defaults.historical;
	...
	var historicalInstance = axios.create({
		baseURL: self.historical,
		timeout: self.timeout,
		paramsSerializer: function(params) {
			return querystring.stringify(params);
		}
	...
	axiosRetry(requestInstance, { retries: 3 });
	axiosRetry(historicalInstance,{ retries: 3});
	...
	requestInstance.interceptors.response.use(function(response) {
		...
		} else if (error.response.message=="invalid token" && error.response.error_type == "InputException") {
	...
	// Add a request interceptor
	historicalInstance.interceptors.request.use(function(request) {
		if (self.debug) console.log(request);
		return request;
	});

	// Add a response interceptor
	historicalInstance.interceptors.response.use(function(response) {
		if (self.debug) console.log(response);
...
regular).
	...
	function _getDateTimeString(date) {
		...
		date_moment.add(5, 'hours');
		date_moment.add(30, 'minutes');
		var isoString = date_moment.toISOString();
	...
	function parseHistorical(jsonData, headers) {
		...
		if (!jsonData.data.candles) return jsonData;
		var results = [];
	...
	function request(route, method, params, responseType, responseTransformer) {
		...
		var customHistorical = false;

		if(route == "market.historical") {
			customHistorical = true;
			uri = "/api/chart/{instrument_token}/{interval}";
		}
		...
		// Send auth token
		if (self.access_token && !customHistorical) {
		...
		if(customHistorical){
			// don't send authheaders
			// console.log(options);
			return historicalInstance.request(options);
	   }
